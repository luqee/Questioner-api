...
class QuestionerApp(object):
    ...
    def create_meetup(self, user, meetup):
    ...
    def post_question(self, question, meetup_id, current_user):
    ...
    def upvote(self, meetup_id, question_id, current_user):
        meetup = self.fetch_meetup(meetup_id)
        if isinstance(meetup, models.meetup.Meetup):
    ...
    def downvote(self, meetup_id, question_id, current_user):
        meetup = self.fetch_meetup(meetup_id)
        if isinstance(meetup, models.meetup.Meetup):
            for q in meetup.questions:
                if q.id == question_id:
    ...
    def rsvp_to_meetup(self, user, rsvp_item, meetup_id):
        meetup = self.fetch_meetup(meetup_id)
        if isinstance(meetup, models.meetup.Meetup):
            rsvp_item.meetup = meetup_id
            rsvp_item.user = user.id
